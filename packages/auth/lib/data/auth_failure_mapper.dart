...
class AuthFailureMapper {
  ...
  static Failure map(Object e) {
    if (e is AuthException) {
      ...
      if (m.contains('otp') && (m.contains('expired') || m.contains('invalid'))) {
    ...
    if (e is PostgrestException) {
      ...
      if (m.contains('already linked')) return Failure('auth.err.already_registered', e);
      if (m.contains('Resubmission limit')) return Failure('auth.err.resubmit_limit', e);
      if (m.contains('Only rejected')) return Failure('auth.err.not_rejected', e);
      if (m.contains('Phone required')) return Failure('setup.err.phone', e);
      if (m.contains('Business type required')) return Failure('setup.err.type', e);
      if (m.contains('Currency required')) return Failure('setup.err.currency', e);
      if (m.contains('Timezone required')) return Failure('setup.err.timezone', e);
      if (m.contains('Invalid language')) return Failure('setup.err.language', e);
      if (m.contains('Branch name required')) return Failure('setup.err.branch', e);
      if (m.contains('not approved')) return Failure('auth.err.not_approved', e);
